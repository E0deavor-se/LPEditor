...
public class AiGenerateDesignService
{
    private readonly IAiChatClient _chatClient;
    private readonly AiDesignValidator _validator;
    private readonly AiOptions _options;
    private readonly LPEditorApp.Utils.ILogger _logger;

    public AiGenerateDesignService(IAiChatClient chatClient, AiDesignValidator validator, IOptions<AiOptions> options, LPEditorApp.Utils.ILogger logger)
    {
    ...
    public async Task<AiDesignOutcome> GenerateDesignAsync(AiDesignRequest request, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(_options.ApiKey))
        {
            return AiDesignOutcome.Failed("AIの設定が未完了です。管理者にお問い合わせください。", new List<string> { "api key missing" });
        }
        ...
        for (var attempt = 1; attempt <= maxRetry + 1; attempt++)
        {
            ...
            try
            {
                ...
                if (validation.IsValid)
                {
                    ...
                    {
                        return AiDesignOutcome.Success(normalized, validation.Warnings, content);
                    }
        ...
        return AiDesignOutcome.Failed(userMessage ?? "AI生成に失敗しました。入力内容を見直して再度お試しください。", errors);
    }

    private List<OpenAiMessage> BuildMessages(AiDesignRequest request, List<string> errors, bool isRetry)
    {
        var system = @"
あなたはLPデザインの設計アシスタントです。
出力はJSONのみ。コードフェンスや説明は絶対に付けない。
必ず以下のJSON Schemaに完全準拠し、未知のフィールドは入れない。
色は #RRGGBB で指定し、読みやすさを優先する。
参考URLがあっても完全コピーはしない。雰囲気/要素抽出のみ。

Schema:
{
  ""version"": 1,
  ""designType"": ""ranking|municipality_rebate|municipality_lottery|coupon|point_lottery"",
  ""theme"": {
    ""primary"": ""#RRGGBB"",
    ""secondary"": ""#RRGGBB"",
    ""accent"": ""#RRGGBB"",
    ...
    ""radius"": 16,
    ""shadow"": ""soft|none"",
    ""font"": ""system|rounded|gothic"",
    ""ctaStyle"": ""solid|outline|gradient""
  },
  ""layout"": {
    ""container"": ""centered|wide"",
    ""hero"": ""split|stacked|poster"",
    ""sectionStyle"": ""card|band|flat"",
    ""headingStyle"": ""pill|underline|bold"",
    ""offerStyle"": ""cardGrid|singleCard"",
    ""howtoStyle"": ""steps|timeline"",
    ""notesStyle"": ""accordion|boxed"",
    ""rankingStyle"": ""table|cards|podium""
  }
}
";
...
";
    ...
    private string ResolveDesignModel()
    {
        if (!string.IsNullOrWhiteSpace(_options.ModelDesignSpec))
        {
            return _options.ModelDesignSpec;
        }
    ...
    private void LogFailure(AiDesignRequest request, string raw, List<string> errors, int attempt, string reason)
    {
        ...
        _logger.Warn($"[AI-Design] {reason} (attempt {attempt}): {errorText}");
        _logger.Warn($"[AI-Design] input: {maskedInput}");
        if (!string.IsNullOrWhiteSpace(maskedRaw))
        {
            _logger.Warn($"[AI-Design] response: {maskedRaw}");
        }
    }
...
public record AiDesignOutcome(bool IsSuccess, LpDesignSpec? Spec, IReadOnlyList<string> Warnings, string? RawJson, IReadOnlyList<string> Errors, string? UserMessage)
{
    public static AiDesignOutcome Success(LpDesignSpec spec, IReadOnlyList<string> warnings, string raw)
        => new(true, spec, warnings, raw, Array.Empty<string>(), null);

    public static AiDesignOutcome Failed(string userMessage, IReadOnlyList<string> errors)
